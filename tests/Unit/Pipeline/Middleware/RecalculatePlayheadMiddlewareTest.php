...
final class RecalculatePlayheadMiddlewareTest extends TestCase
{
    ...
    public function testRecalculatePlayheadWithStreamHeader(): void
    {
        $middleware = new RecalculatePlayheadMiddleware();
        ...
        $message = Message::create($event)
            ->withHeader(new StreamHeader('profile-1', 5, new DateTimeImmutable()));

        $result = $middleware($message);

        self::assertCount(1, $result);
        self::assertSame('profile-1', $result[0]->header(StreamHeader::class)->streamName);
        self::assertSame(1, $result[0]->header(StreamHeader::class)->playhead);
    }

    public function testRecalculateWithoutHeader(): void
    {
        $middleware = new RecalculatePlayheadMiddleware();
